...
--         Full binaries live exclusively in BC16 DocumentObject (C14, C15).
...
CREATE TABLE tblAuditEvent (
    ...
    corrects                UUID            REFERENCES tblAuditEvent (event_id)
    ...
    CONSTRAINT tblAuditEvent_pk PRIMARY KEY (event_id),
    CONSTRAINT tblAuditEvent_aggregate_version_positive
        CHECK (aggregate_version >= 1),
    CONSTRAINT tblAuditEvent_event_version_positive
        CHECK (event_version >= 1)
);

COMMENT ON TABLE tblAuditEvent IS
...
COMMENT ON COLUMN tblAuditEvent.aggregate_version IS
...
COMMENT ON COLUMN tblAuditEvent.causation_id IS
...
COMMENT ON COLUMN tblAuditEvent.payload IS
...
COMMENT ON COLUMN tblAuditEvent.previous_envelope_hash IS
...
-- The trigger fires BEFORE any UPDATE or DELETE on tblAuditEvent and raises an exception,
...
BEGIN
    RAISE EXCEPTION
        'tblAuditEvent is append-only (AE.1, C1, DL-040). '
...
CREATE TRIGGER prevent_audit_modification
    BEFORE UPDATE OR DELETE ON tblAuditEvent
    FOR EACH ROW
    EXECUTE FUNCTION fn_prevent_audit_modification();

COMMENT ON TRIGGER prevent_audit_modification ON tblAuditEvent IS
...
CREATE INDEX idx_audit_aggregate
    ON tblAuditEvent (aggregate_id, aggregate_type);
...
CREATE INDEX idx_audit_correlation
    ON tblAuditEvent (correlation_id);
...
CREATE INDEX idx_audit_actor_id
    ON tblAuditEvent ((actor->>'actor_id'));
...
--         Recipient identity is stored as recipient_identity_id (FK to tblIdentity) —
--         the application resolves the address at dispatch time from the Identity context.
--   ND.3  causation_event_id is a SOFT FK to tblAuditEvent.event_id (not a hard FK).
...
CREATE TABLE tblNotificationDispatch (
    dispatch_id             UUID                        NOT NULL,
    -- Recipient — FK to tblIdentity (BC7/BC8/BC9 identity anchor).
    ...
    provider_ref            TEXT,
    -- Soft FK to tblAuditEvent.event_id — see ND.3 above.
    -- Stored as UUID so queries can join on event_id without a hard FK constraint.
    causation_event_id      UUID,

    CONSTRAINT tblNotificationDispatch_pk PRIMARY KEY (dispatch_id)
);

COMMENT ON TABLE tblNotificationDispatch IS
...
COMMENT ON COLUMN tblNotificationDispatch.dispatch_id IS
    'UUIDv7 PK minted by the notification context at enqueue time.';
COMMENT ON COLUMN tblNotificationDispatch.recipient_identity_id IS
    'FK to tblIdentity (Identity context). Application resolves channel address at dispatch; '
    'raw email / phone number is never persisted here (ND.2, C14, C15).';
COMMENT ON COLUMN tblNotificationDispatch.payload IS
    'Template variable bag ONLY. Raw OTPs, PII (email, phone, PAN, Aadhaar) must NEVER appear '
    'in this column (ND.2, C14, C15, DL-050).';
COMMENT ON COLUMN tblNotificationDispatch.provider_ref IS
...
COMMENT ON COLUMN tblNotificationDispatch.causation_event_id IS
    'UUID of the tblAuditEvent row that caused this notification. '
...
CREATE INDEX idx_notification_recipient
    ON tblNotificationDispatch (recipient_identity_id);
...
CREATE INDEX idx_notification_status
    ON tblNotificationDispatch (status)
...
--   DO.2  Binary content is NEVER inlined into AuditEvent payloads (tblAuditEvent.payload).
...
CREATE TABLE tblDocumentObject (
    ...
    CONSTRAINT tblDocumentObject_pk PRIMARY KEY (doc_hash),
    CONSTRAINT tblDocumentObject_byte_size_positive CHECK (byte_size > 0)
);

COMMENT ON TABLE tblDocumentObject IS
...
COMMENT ON COLUMN tblDocumentObject.doc_hash IS
...
COMMENT ON COLUMN tblDocumentObject.originating_context IS
    'Bounded context that produced and owns this document e.g. bc17_verification, bc19_signing, bc11_compliance.';
COMMENT ON COLUMN tblDocumentObject.originating_aggregate_ref IS
...
COMMENT ON COLUMN tblDocumentObject.encryption_key_ref IS
...
CREATE INDEX idx_document_originating
    ON tblDocumentObject (originating_context, originating_aggregate_ref);
...
--         the vendor. Only the hash is stored here; the binary lives in tblDocumentObject.
...
CREATE TABLE tblVerification (
    ...
    -- SHA-256 of verbatim vendor response stored in tblDocumentObject (V.3).
    ...
    CONSTRAINT tblVerification_pk PRIMARY KEY (verification_id)
);

COMMENT ON TABLE tblVerification IS
    ...
    'Verbatim vendor payload stored in tblDocumentObject (BC16) — only hash here (V.3).';
COMMENT ON COLUMN tblVerification.verification_id IS
    'UUIDv7 PK = client_request_id sent to the aggregator. Acts as the idempotency key on the vendor side.';
COMMENT ON COLUMN tblVerification.subject_id IS
    'UUID of the entity being verified: investor_account_id, supplier_account_id, buyer_account_id, etc.';
COMMENT ON COLUMN tblVerification.vendor_payload_hash IS
    'SHA-256 of verbatim vendor JSON response. Full payload archived in tblDocumentObject. '
    'NULL until vendor responds. Enables re-parsing without re-calling the vendor.';
COMMENT ON COLUMN tblVerification.extracted_fields IS
...
COMMENT ON COLUMN tblVerification.ttl_until IS
...
COMMENT ON COLUMN tblVerification.failure_class IS
    'Error classification for failed/manual_fallback rows (A2 §1.5). '
    'E.g. vendor_5xx, timeout, business_mismatch, uidai_outage.';
COMMENT ON COLUMN tblVerification.signature_verified_at IS
...
CREATE INDEX idx_verification_subject_api
    ON tblVerification (subject_id, api_name);
...
CREATE TABLE tblVendorInstruction (
    ...
    -- Soft FK to tblPayoutInstruction (BC4) — cross-context reference by identity (VI.4).
    ...
    -- SHA-256 of verbatim vendor response body. Full payload in tblDocumentObject (BC16).
    vendor_payload_hash         BYTEA,
    -- When the inbound webhook HMAC was verified (VI.2, C10).
    hmac_verified_at            TIMESTAMPTZ,

    CONSTRAINT tblVendorInstruction_pk PRIMARY KEY (vendor_instruction_id)
);

COMMENT ON TABLE tblVendorInstruction IS
    ...
    'Verbatim vendor response archived in tblDocumentObject; only hash stored here. '
    'Webhook deduplication via UNIQUE vendor_event_id (VI.3, C22, G1).';
COMMENT ON COLUMN tblVendorInstruction.vendor_instruction_id IS
    'UUIDv7 PK = client_instruction_id sent to escrow. Idempotency key: same UUID safe to retry (VI.1, C9).';
COMMENT ON COLUMN tblVendorInstruction.linked_payout_instruction_id IS
    'UUID of the originating PayoutInstruction (BC4). '
    'SOFT FK — no REFERENCES constraint. Cross-context reference by identity only (B1 §2, VI.4).';
COMMENT ON COLUMN tblVendorInstruction.vendor_event_id IS
...
COMMENT ON COLUMN tblVendorInstruction.vendor_payload_hash IS
    'SHA-256 of verbatim escrow vendor response. Full payload archived in tblDocumentObject (BC16).';
COMMENT ON COLUMN tblVendorInstruction.hmac_verified_at IS
...
CREATE INDEX idx_vinstr_payout_link
    ON tblVendorInstruction (linked_payout_instruction_id)
    WHERE linked_payout_instruction_id IS NOT NULL;
COMMENT ON INDEX idx_vinstr_payout_link IS
    'Lookup all escrow instructions for a given PayoutInstruction (BC4 reconciliation path).';

CREATE INDEX idx_vinstr_status
    ON tblVendorInstruction (status)
...
--   IO.3  corrects references an earlier InflowObservation that this row supersedes in a
...
CREATE TABLE tblInflowObservation (
    -- UUIDv7 PK.
    ...
    -- References a prior InflowObservation this row corrects (IO.3, B2 P6).
    corrects                UUID,

    CONSTRAINT tblInflowObservation_pk PRIMARY KEY (inflow_id),
    CONSTRAINT tblInflowObservation_utr_unique UNIQUE (utr),
    CONSTRAINT tblInflowObservation_vendor_event_id_unique UNIQUE (vendor_event_id),
    CONSTRAINT tblInflowObservation_amount_positive CHECK (amount > 0)
);

COMMENT ON TABLE tblInflowObservation IS
...
COMMENT ON COLUMN tblInflowObservation.inflow_id IS
    'UUIDv7 PK minted by the Banking ACL at webhook ingress.';
COMMENT ON COLUMN tblInflowObservation.va_id IS
...
COMMENT ON COLUMN tblInflowObservation.vendor_event_id IS
    'Vendor-assigned event identifier. UNIQUE constraint enforces deduplication at storage layer (IO.2, C22).';
COMMENT ON COLUMN tblInflowObservation.corrects IS
    'UUID of the InflowObservation this row supersedes in a corrective reconciliation (IO.3, B2 P6). '
...
CREATE INDEX idx_inflow_va
    ON tblInflowObservation (va_id, observed_at DESC);
...
CREATE INDEX idx_inflow_status
    ON tblInflowObservation (status)
...
--         Not a hard FK — cross-context reference by identity (B1 §2).
--   VS.7  doc_hash is a soft FK to tblDocumentObject (BC16) — the document being signed.
--         Not a hard FK — document may not exist in tblDocumentObject at session initiation
--         (e.g. stamped PDF is created on signing completion). Application ensures consistency.

CREATE TABLE tblVendorSignatureRequest (
    ...
    -- SHA-256 of the document being signed — soft FK to tblDocumentObject (VS.7).
    doc_hash                BYTEA               NOT NULL,
    ...
    CONSTRAINT tblVendorSignatureRequest_pk
        PRIMARY KEY (vsr_id),
    CONSTRAINT tblVendorSignatureRequest_unique_request_doc
        UNIQUE (signature_request_id, doc_hash),
    CONSTRAINT tblVendorSignatureRequest_cert_serial_only_on_completed
        CHECK (cert_serial IS NULL OR status = 'completed'),
    CONSTRAINT tblVendorSignatureRequest_retry_count_max
        CHECK (retry_count >= 0 AND retry_count <= 3)
);

COMMENT ON TABLE tblVendorSignatureRequest IS
...
COMMENT ON COLUMN tblVendorSignatureRequest.vsr_id IS
    'UUIDv7 PK = the session token sent to the signing vendor. Idempotency key for the vendor session (VS.1).';
COMMENT ON COLUMN tblVendorSignatureRequest.signature_request_id IS
...
COMMENT ON COLUMN tblVendorSignatureRequest.doc_hash IS
    'SHA-256 of the document submitted for signing. '
    'SOFT FK to tblDocumentObject (BC16) — no REFERENCES constraint (VS.7). '
    'Together with signature_request_id, forms the UNIQUE document-level dedup key (VS.2).';
COMMENT ON COLUMN tblVendorSignatureRequest.signer_ref IS
...
COMMENT ON COLUMN tblVendorSignatureRequest.cert_serial IS
...
COMMENT ON COLUMN tblVendorSignatureRequest.hmac_verified_at IS
...
COMMENT ON COLUMN tblVendorSignatureRequest.retry_count IS
...
CREATE INDEX idx_vsr_signature_request
    ON tblVendorSignatureRequest (signature_request_id);
...
CREATE INDEX idx_vsr_status
    ON tblVendorSignatureRequest (status)
...
CREATE INDEX idx_audit_chain_verify
    ON tblAuditEvent (recorded_at ASC, event_id ASC);
...
CREATE INDEX idx_notification_causation
    ON tblNotificationDispatch (causation_event_id)
...
CREATE INDEX idx_inflow_corrects
    ON tblInflowObservation (corrects)
