...
CREATE EXTENSION IF NOT EXISTS citext;   -- case-insensitive text; used by tblIdentity.email (03_auth.sql)
...
-- Prefixed by BC number per naming convention.
-- =============================================================================
...
CREATE TYPE bc1_invoice_status AS ENUM (
    'submitted',
...
CREATE TYPE bc1_listing_status AS ENUM (
    'draft',
    ...
    'seriously_delayed',
    'under_adjudication',
    -- outcome states (aligned with bc3_default_outcome and bc6_maturity_outcome)
    'disputed',
    'dilution',
    'fraud',
    'defaulted',
...
CREATE TYPE bc1_terminal_outcome AS ENUM (
    'distributed',
    'funding_failed_refunded',
    'cancelled_pre_disbursement',
    'defaulted'
...
CREATE TYPE bc2_subscription_status AS ENUM (
    'committed',
    'funds_pending',
    'funds_received',
    'confirmed',
...
CREATE TYPE bc3_tenor_bucket AS ENUM (
    'lte_30d',
    '31_60d',
    '61_90d',
    '91_180d'
...
CREATE TYPE bc3_default_case_status AS ENUM (
    'requested',
    'under_adjudication',
    'classified'
);

CREATE TYPE bc3_default_outcome AS ENUM (
    'disputed',
    'dilution',
    'fraud',
    'defaulted',
...
CREATE TYPE bc4_virtual_account_status AS ENUM (
    'requested',
    'created',
    'closed'
);

CREATE TYPE bc4_payout_kind AS ENUM (
    'disbursement',
    'distribution',
    'refund'
);

CREATE TYPE bc4_payout_status AS ENUM (
    'drafted',
    'approved',
    'sent',
    'executed',
...
CREATE TYPE bc4_recon_ledger_status AS ENUM (
    'open',
    'reconciling',
    'completed',
...
CREATE TYPE bc4_remediation_trigger AS ENUM (
    'leg_failed',
    'inflow_unmatched',
    'discrepancy',
...
CREATE TYPE bc4_remediation_status AS ENUM (
    'open',
    'in_progress',
    'resolved'
);
...
CREATE TYPE bc5_master_agreement_party_type AS ENUM (
    'supplier',
    'investor'
);

CREATE TYPE bc5_master_agreement_kind AS ENUM (
    'MAA',
    'MIA'
);

CREATE TYPE bc5_master_agreement_status AS ENUM (
    'initiated',
    'signed',
    'stamped',
    'failed'
...
CREATE TYPE bc5_assignment_set_status AS ENUM (
    'requested',
    'in_progress',
    'all_signed',
    'incomplete'
...
CREATE TYPE bc5_assignment_leg_status AS ENUM (
    'pending',
    'initiated',
    'signed',
    'failed'
...
CREATE TYPE bc5_signature_request_status AS ENUM (
    'initiated',
    'completed',
    'failed'
);

CREATE TYPE bc5_signer_type AS ENUM (
    'supplier_signatory',
    'investor',
...
CREATE TYPE bc6_delay_status AS ENUM (
    'on_track',
    'mildly_delayed',
    'delayed',
    'seriously_delayed',
...
CREATE TYPE bc6_maturity_outcome AS ENUM (
    'disputed',
    'dilution',
    'fraud',
    'defaulted',
...
CREATE TYPE bc6_collections_action_type AS ENUM (
    'phone_followup',
    'email_notice',
    'panel_lawyer_letter',
    'other'
...
CREATE TYPE bc6_claim_type AS ENUM (
    'dilution',
    'fraud',
    'dispute'
);

CREATE TYPE bc6_claim_status AS ENUM (
    'raised',
    'under_adjudication',
    'resolved'
);
...
-- bc1_invoice
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc1_invoice
...
CREATE TABLE bc1_invoice (
    ...
    status              bc1_invoice_status NOT NULL DEFAULT 'submitted',
    -- check_outcomes: map<check_name → {outcome, detail}> (DL-027, C24)
    check_outcomes      JSONB           NOT NULL DEFAULT '{}',
    aggregate_version   INT             NOT NULL DEFAULT 1,
    created_at          TIMESTAMPTZ     NOT NULL DEFAULT now(),
    updated_at          TIMESTAMPTZ     NOT NULL DEFAULT now(),

    CONSTRAINT bc1_invoice_pk PRIMARY KEY (invoice_id),
    CONSTRAINT bc1_invoice_face_value_positive CHECK (face_value > 0),
    CONSTRAINT bc1_invoice_tenor_range CHECK (tenor_days BETWEEN 1 AND 180)
);

COMMENT ON TABLE bc1_invoice IS
...
COMMENT ON COLUMN bc1_invoice.invoice_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc1_invoice.irn IS
...
COMMENT ON COLUMN bc1_invoice.invoice_date IS
...
CREATE UNIQUE INDEX bc1_invoice_irn_supplier_uidx
    ON bc1_invoice (supplier_id, irn)
    WHERE irn IS NOT NULL;

-- Partial unique index: manual-entry uniqueness (INV.1 fallback)
CREATE UNIQUE INDEX bc1_invoice_manual_uidx
    ON bc1_invoice (supplier_id, buyer_id, invoice_number, face_value, tenor_days)
    WHERE irn IS NULL;

CREATE INDEX bc1_invoice_supplier_idx ON bc1_invoice (supplier_id);
CREATE INDEX bc1_invoice_buyer_idx    ON bc1_invoice (buyer_id);
CREATE INDEX bc1_invoice_status_idx   ON bc1_invoice (status);


-- ---------------------------------------------------------------------------
-- bc1_listing
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc1_listing
...
--   L.12  va_id is set exactly once (at GoneLive-driven BC4 VirtualAccount
...
CREATE TABLE bc1_listing (
    listing_id                          UUID                NOT NULL,
    invoice_id                          UUID                NOT NULL,
    supplier_id                         UUID                NOT NULL,
    buyer_id                            UUID                NOT NULL,
    status                              bc1_listing_status  NOT NULL DEFAULT 'draft',
    ...
    -- VA reference (set once at GoneLive; FK to bc4_virtual_account) (C8, DL-043)
    va_id                               UUID,
    ...
    -- Terminal metadata
    terminal_outcome                    bc1_terminal_outcome,
    ...
    CONSTRAINT bc1_listing_pk PRIMARY KEY (listing_id),
    CONSTRAINT bc1_listing_invoice_fk
        FOREIGN KEY (invoice_id) REFERENCES bc1_invoice (invoice_id),
    CONSTRAINT bc1_listing_committed_lte_target
        CHECK (committed_total <= COALESCE(funding_target, committed_total)),
    CONSTRAINT bc1_listing_funding_target_positive
        CHECK (funding_target IS NULL OR funding_target > 0),
    CONSTRAINT bc1_listing_committed_nonneg
        CHECK (committed_total >= 0),
    CONSTRAINT bc1_listing_golive_maker_ne_checker
        CHECK (golive_maker_id IS NULL
               OR golive_checker_id IS NULL
               OR golive_maker_id <> golive_checker_id)
);

COMMENT ON TABLE bc1_listing IS
...
COMMENT ON COLUMN bc1_listing.listing_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc1_listing.pricing_snapshot IS
...
COMMENT ON COLUMN bc1_listing.buyer_limit_headroom_snapshot IS
    'Buyer credit limit headroom in paise at SnapshotTaken. '
    'Immutable after ready_for_review (G20, L.3).';
COMMENT ON COLUMN bc1_listing.supplier_exposure_cap_snapshot IS
    'Supplier exposure cap in paise at SnapshotTaken. '
    'Immutable after ready_for_review (G20, L.3).';
COMMENT ON COLUMN bc1_listing.funding_target IS
...
COMMENT ON COLUMN bc1_listing.committed_total IS
...
COMMENT ON COLUMN bc1_listing.funding_window_close_at IS
    ...
    '(DL-017, C12, L.8)';
COMMENT ON COLUMN bc1_listing.va_id IS
    'Reference to bc4_virtual_account. Set exactly once at GoneLive. '
    'Never cleared until terminal Close. (C8, DL-043, L.12)';
COMMENT ON COLUMN bc1_listing.all_signed IS
    'Set to TRUE by subscriber to BC5 AssignmentSet.AllSigned. '
    'Gates DisbursementGateOpened (C27, DL-002, L.5).';
COMMENT ON COLUMN bc1_listing.terminal_outcome IS
    'Populated only on Close. Identifies terminal path taken.';
COMMENT ON COLUMN bc1_listing.golive_maker_id IS
    'Admin user who prepared the GoneLive action (maker in maker-checker). (C4)';
COMMENT ON COLUMN bc1_listing.golive_checker_id IS
    'Admin user (Treasury & Settlement role) who approved GoneLive. '
    'Must differ from golive_maker_id. (C4, C7, L.4)';
COMMENT ON COLUMN bc1_listing.golive_mfa_assertion_id IS
    'MFA assertion ref for golive_checker_id at approval time. (C7, L.4)';
COMMENT ON COLUMN bc1_listing.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc1_listing_status_idx      ON bc1_listing (status);
CREATE INDEX bc1_listing_supplier_idx    ON bc1_listing (supplier_id);
CREATE INDEX bc1_listing_buyer_idx       ON bc1_listing (buyer_id);
CREATE INDEX bc1_listing_invoice_idx     ON bc1_listing (invoice_id);
CREATE INDEX bc1_listing_window_close_idx ON bc1_listing (funding_window_close_at)
    WHERE status = 'live';
...
-- bc2_subscription
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc2_subscription
...
CREATE TABLE bc2_subscription (
    subscription_id                 UUID                        NOT NULL,
    listing_id                      UUID                        NOT NULL,
    investor_id                     UUID                        NOT NULL,
    -- amount in paise; minimum 1,000,000 (Rs 10,000). (DL-007, S.1)
    amount                          BIGINT                      NOT NULL,
    status                          bc2_subscription_status     NOT NULL DEFAULT 'committed',
    ...
    CONSTRAINT bc2_subscription_pk PRIMARY KEY (subscription_id),
    CONSTRAINT bc2_subscription_listing_fk
        FOREIGN KEY (listing_id) REFERENCES bc1_listing (listing_id),

    -- S.1: minimum ticket Rs 10,000 = 1,000,000 paise (DL-007)
    CONSTRAINT bc2_subscription_min_amount
        CHECK (amount >= 1000000),
    CONSTRAINT bc2_subscription_amount_positive
        CHECK (amount > 0),
    CONSTRAINT bc2_subscription_expected_inflow_positive
        CHECK (expected_inflow_amount > 0),

    -- S.6: one subscription per (listing_id, investor_id)
    CONSTRAINT bc2_subscription_listing_investor_uq
    ...
    CONSTRAINT bc2_subscription_distribution_net_check
        CHECK (
...
COMMENT ON TABLE bc2_subscription IS
...
COMMENT ON COLUMN bc2_subscription.subscription_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc2_subscription.amount IS
    'Committed investment amount in paise. Minimum 1,000,000 (Rs 10,000). '
    '(DL-007, S.1)';
COMMENT ON COLUMN bc2_subscription.expected_inflow_amount IS
...
COMMENT ON COLUMN bc2_subscription.actual_inflow_txn_ref IS
...
COMMENT ON COLUMN bc2_subscription.concentration_warnings_at_commit IS
    'JSONB array of soft concentration warnings recorded at commit time. '
    'Audit record only — not a hard block. (DL-011, S.8)';
COMMENT ON COLUMN bc2_subscription.wallet_attribution IS
    'DORMANT in Phase 1. Reserved for Phase 2 wallet/source-of-funds '
    'attribution. App layer must not set this in Phase 1. (C25, C26)';
COMMENT ON COLUMN bc2_subscription.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc2_subscription_listing_idx  ON bc2_subscription (listing_id);
CREATE INDEX bc2_subscription_investor_idx ON bc2_subscription (investor_id);
CREATE INDEX bc2_subscription_status_idx   ON bc2_subscription (status);
...
-- ---------------------------------------------------------------------------
-- bc3_buyer_credit_profile
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc3_buyer_credit_profile
...
CREATE TABLE bc3_buyer_credit_profile (
    buyer_id                UUID        NOT NULL,
    ...
    CONSTRAINT bc3_buyer_credit_profile_pk PRIMARY KEY (buyer_id),
    CONSTRAINT bc3_buyer_credit_limit_positive CHECK (credit_limit > 0),
    CONSTRAINT bc3_buyer_tenor_cap_range CHECK (tenor_cap_days BETWEEN 1 AND 180)
);

COMMENT ON TABLE bc3_buyer_credit_profile IS
...
COMMENT ON COLUMN bc3_buyer_credit_profile.buyer_id IS
    'UUIDv7. Shared identity with BC9 BuyerAccount. BC3 holds the credit-side '
    'projection only. Cross-context reference by identity.';
COMMENT ON COLUMN bc3_buyer_credit_profile.credit_limit IS
    'Buyer aggregate credit limit in paise. Must be > 0. (DL-022)';
COMMENT ON COLUMN bc3_buyer_credit_profile.tenor_cap_days IS
    'Maximum allowable tenor in days for this buyer. Range [1, 180]. (DL-022)';
COMMENT ON COLUMN bc3_buyer_credit_profile.four_eyes_approval_ref IS
...
COMMENT ON COLUMN bc3_buyer_credit_profile.second_approver_id IS
...
COMMENT ON COLUMN bc3_buyer_credit_profile.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc3_buyer_credit_profile_sector_idx ON bc3_buyer_credit_profile (sector);


-- ---------------------------------------------------------------------------
-- bc3_supplier_credit_profile
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc3_supplier_credit_profile
...
CREATE TABLE bc3_supplier_credit_profile (
    supplier_id             UUID        NOT NULL,
    ...
    CONSTRAINT bc3_supplier_credit_profile_pk PRIMARY KEY (supplier_id),
    CONSTRAINT bc3_supplier_exposure_cap_nonneg CHECK (exposure_cap >= 0)
);

COMMENT ON TABLE bc3_supplier_credit_profile IS
...
COMMENT ON COLUMN bc3_supplier_credit_profile.supplier_id IS
    'UUIDv7. Shared identity with BC8 SupplierAccount. Cross-context by identity.';
COMMENT ON COLUMN bc3_supplier_credit_profile.exposure_cap IS
    'Aggregate supplier exposure cap in paise. Zero means no new listings '
    'permitted. (DL-022, SCP.1)';
COMMENT ON COLUMN bc3_supplier_credit_profile.second_approver_id IS
    'Required when exposure_cap > 10,000,000,000 paise (Rs 1 Cr). '
    'App layer must enforce second_approver_id ≠ first approver. (C6, SCP.2)';
COMMENT ON COLUMN bc3_supplier_credit_profile.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';
...
-- bc3_pricing_policy
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc3_pricing_policy
...
--   PB.3  Past PricingPolicy records are immutable. A re-pricing creates a
...
CREATE TABLE bc3_pricing_policy (
    pricing_band_id         UUID                NOT NULL,
    buyer_id                UUID                NOT NULL,
    tenor_bucket            bc3_tenor_bucket    NOT NULL,
    ...
    CONSTRAINT bc3_pricing_policy_pk PRIMARY KEY (pricing_band_id),
    CONSTRAINT bc3_pricing_policy_superseded_fk
        FOREIGN KEY (superseded_by) REFERENCES bc3_pricing_policy (pricing_band_id),
    CONSTRAINT bc3_pricing_rate_min_positive CHECK (rate_range_min_bps > 0),
    CONSTRAINT bc3_pricing_rate_range_valid
        CHECK (rate_range_min_bps <= rate_range_max_bps),
    CONSTRAINT bc3_pricing_fee_nonneg CHECK (fee_bps >= 0)
);

COMMENT ON TABLE bc3_pricing_policy IS
...
COMMENT ON COLUMN bc3_pricing_policy.pricing_band_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc3_pricing_policy.rate_range_min_bps IS
    'Minimum discount rate in basis points (bps). Must be > 0. (DL-024)';
COMMENT ON COLUMN bc3_pricing_policy.rate_range_max_bps IS
    'Maximum discount rate in basis points. Must be ≥ rate_range_min_bps. (DL-024)';
COMMENT ON COLUMN bc3_pricing_policy.fee_bps IS
    'Platform fee in basis points applied flat on face value. Must be ≥ 0. (DL-024)';
COMMENT ON COLUMN bc3_pricing_policy.effective_from IS
    'BusinessDate from which this band is effective.';
COMMENT ON COLUMN bc3_pricing_policy.superseded_by IS
...
COMMENT ON COLUMN bc3_pricing_policy.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

-- PB.2: one active band per (buyer_id, tenor_bucket). (DL-024)
CREATE UNIQUE INDEX bc3_pricing_policy_active_uidx
    ON bc3_pricing_policy (buyer_id, tenor_bucket)
    WHERE superseded_by IS NULL;

CREATE INDEX bc3_pricing_policy_buyer_idx ON bc3_pricing_policy (buyer_id);


-- ---------------------------------------------------------------------------
-- bc3_default_case
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc3_default_case
...
CREATE TABLE bc3_default_case (
    case_id                 UUID                        NOT NULL,
    listing_id              UUID                        NOT NULL,
    status                  bc3_default_case_status     NOT NULL DEFAULT 'requested',
    outcome                 bc3_default_outcome,
    ...
    CONSTRAINT bc3_default_case_pk PRIMARY KEY (case_id),
    CONSTRAINT bc3_default_case_listing_fk
        FOREIGN KEY (listing_id) REFERENCES bc1_listing (listing_id),
    CONSTRAINT bc3_default_case_corrects_fk
        FOREIGN KEY (corrects_case_id) REFERENCES bc3_default_case (case_id),
    CONSTRAINT bc3_default_case_maker_ne_checker
        CHECK (maker_id IS NULL OR checker_id IS NULL OR maker_id <> checker_id)
);

COMMENT ON TABLE bc3_default_case IS
...
COMMENT ON COLUMN bc3_default_case.case_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc3_default_case.outcome IS
    'Set exactly once on Classify command. NULL until classified. '
    'Re-classification requires a new row (DC.3, G23).';
COMMENT ON COLUMN bc3_default_case.four_eyes_approval_ref IS
    'Required when listing exposure > 10,000,000,000 paise (Rs 1 Cr). '
    'References a FourEyesApproval envelope. (C6, DL-023, DC.2)';
COMMENT ON COLUMN bc3_default_case.corrects_case_id IS
    'Points to prior case_id when this row is a re-classification. '
    'NULL on initial case. (DC.3, G23)';
COMMENT ON COLUMN bc3_default_case.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc3_default_case_listing_idx ON bc3_default_case (listing_id);
CREATE INDEX bc3_default_case_status_idx  ON bc3_default_case (status);
...
-- bc4_virtual_account
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc4_virtual_account
...
--        are confirmed. Discrepancy at EoD triggers RemediationCase. (C23, G6)
...
CREATE TABLE bc4_virtual_account (
    va_id                       UUID                        NOT NULL,
    listing_id                  UUID                        NOT NULL,
    status                      bc4_virtual_account_status  NOT NULL DEFAULT 'requested',
    ...
    CONSTRAINT bc4_virtual_account_pk PRIMARY KEY (va_id),
    CONSTRAINT bc4_virtual_account_listing_fk
        FOREIGN KEY (listing_id) REFERENCES bc1_listing (listing_id),
    -- V.1: one VA per listing (C8, DL-043)
    CONSTRAINT bc4_virtual_account_listing_uidx UNIQUE (listing_id),
    CONSTRAINT bc4_va_expected_inflow_nonneg CHECK (expected_inflow_total >= 0),
    CONSTRAINT bc4_va_observed_inflow_nonneg CHECK (observed_inflow_total >= 0)
);

COMMENT ON TABLE bc4_virtual_account IS
...
COMMENT ON COLUMN bc4_virtual_account.va_id IS
    'UUIDv7 primary key (platform-minted). Bank-side IFSC/account_no are '
    'observed values set on webhook receipt from BC18.';
COMMENT ON COLUMN bc4_virtual_account.ifsc IS
    'Bank IFSC code of the virtual account. Set on BC18 Va.LifecycleObserved.';
COMMENT ON COLUMN bc4_virtual_account.account_no IS
    'Virtual account number. Set on BC18 Va.LifecycleObserved.';
COMMENT ON COLUMN bc4_virtual_account.expected_inflow_total IS
    'Sum of active subscription amounts + buyer maturity expected, in paise. '
    'Updated as subscriptions are confirmed. (V.2, C23)';
COMMENT ON COLUMN bc4_virtual_account.observed_inflow_total IS
    'Sum of reconciled inflows on this VA, in paise. '
    'Updated by EoD reconciler. Discrepancy vs expected triggers RemediationCase. '
    '(V.2, C23, G6)';
COMMENT ON COLUMN bc4_virtual_account.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc4_virtual_account_status_idx  ON bc4_virtual_account (status);


-- ---------------------------------------------------------------------------
-- bc4_payout_instruction
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc4_payout_instruction
...
--         RemediationCase that is resolved. (G11)
...
CREATE TABLE bc4_payout_instruction (
    payout_instruction_id       UUID                    NOT NULL,
    kind                        bc4_payout_kind         NOT NULL,
    -- listing_id for disbursement/distribution; subscription_id for refund.
    listing_id                  UUID,
    subscription_id             UUID,
    status                      bc4_payout_status       NOT NULL DEFAULT 'drafted',
    ...
    CONSTRAINT bc4_payout_instruction_pk PRIMARY KEY (payout_instruction_id),
    CONSTRAINT bc4_payout_instruction_listing_fk
        FOREIGN KEY (listing_id) REFERENCES bc1_listing (listing_id),
    CONSTRAINT bc4_payout_instruction_subscription_fk
        FOREIGN KEY (subscription_id) REFERENCES bc2_subscription (subscription_id),
    -- PI.5: maker ≠ checker (C4)
    CONSTRAINT bc4_payout_maker_ne_checker
        CHECK (checker_id IS NULL OR maker_id <> checker_id),
    CONSTRAINT bc4_payout_gross_positive CHECK (gross_amount > 0),
    CONSTRAINT bc4_payout_net_positive   CHECK (net_amount > 0),
    CONSTRAINT bc4_payout_fee_nonneg     CHECK (fee_amount >= 0),
    CONSTRAINT bc4_payout_tds_nonneg     CHECK (total_tds_amount IS NULL OR total_tds_amount >= 0)
);

COMMENT ON TABLE bc4_payout_instruction IS
...
COMMENT ON COLUMN bc4_payout_instruction.gross_amount IS
    'Total gross amount in paise before TDS and fee deductions.';
COMMENT ON COLUMN bc4_payout_instruction.net_amount IS
    'Net amount in paise after TDS and fee (for distribution = Σ leg net).';
COMMENT ON COLUMN bc4_payout_instruction.fee_amount IS
    'Platform fee in paise deducted from gross.';
COMMENT ON COLUMN bc4_payout_instruction.total_tds_amount IS
    'Total TDS in paise (distribution kind only). (DL-045, G4)';
COMMENT ON COLUMN bc4_payout_instruction.maker_id IS
    'Admin user (Treasury & Settlement) who drafted the instruction. (C4)';
COMMENT ON COLUMN bc4_payout_instruction.checker_id IS
    'Admin user (Treasury & Settlement) who approved the instruction. '
    'Must differ from maker_id (CHECK constraint). (C4, C7, PI.5)';
COMMENT ON COLUMN bc4_payout_instruction.checker_mfa_assertion_id IS
    'MFA assertion reference for checker at approval time. (C7, PI.5)';
COMMENT ON COLUMN bc4_payout_instruction.instruction_sla_date IS
    'BusinessDate by which instruction must be sent (T+1 SLA). (C11, PI.8)';
COMMENT ON COLUMN bc4_payout_instruction.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc4_payout_instruction_listing_idx      ON bc4_payout_instruction (listing_id);
CREATE INDEX bc4_payout_instruction_subscription_idx ON bc4_payout_instruction (subscription_id);
CREATE INDEX bc4_payout_instruction_status_idx       ON bc4_payout_instruction (status);
CREATE INDEX bc4_payout_instruction_kind_idx         ON bc4_payout_instruction (kind);


-- ---------------------------------------------------------------------------
-- bc4_reconciliation_ledger
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc4_reconciliation_ledger
...
CREATE TABLE bc4_reconciliation_ledger (
    -- Natural identity: one ledger per business date. (RL.1, C23)
    business_date           DATE                        NOT NULL,
    status                  bc4_recon_ledger_status     NOT NULL DEFAULT 'open',
    ...
    CONSTRAINT bc4_reconciliation_ledger_pk PRIMARY KEY (business_date),
    CONSTRAINT bc4_recon_inflows_matched_nonneg   CHECK (inflows_matched >= 0),
    CONSTRAINT bc4_recon_inflows_unmatched_nonneg CHECK (inflows_unmatched >= 0)
);

COMMENT ON TABLE bc4_reconciliation_ledger IS
    ...
    'Cannot complete while discrepancies lack RemediationCases. (G6, G11)';

COMMENT ON COLUMN bc4_reconciliation_ledger.business_date IS
    'Natural primary key. One ledger per business day (RL.1, C23).';
COMMENT ON COLUMN bc4_reconciliation_ledger.master_statement_hash IS
...
COMMENT ON COLUMN bc4_reconciliation_ledger.discrepancies IS
    'JSONB array of {va_id, expected_paise, observed_paise} discrepancy records. '
    'Each entry must have a linked RemediationCase before ledger can complete. (G11)';
COMMENT ON COLUMN bc4_reconciliation_ledger.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc4_recon_ledger_status_idx ON bc4_reconciliation_ledger (status);


-- ---------------------------------------------------------------------------
-- bc4_remediation_case
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc4_remediation_case
...
--         RemediationCase — failed legs cannot be silently dropped. (G11)
-- ---------------------------------------------------------------------------

CREATE TABLE bc4_remediation_case (
    case_id                     UUID                        NOT NULL,
    trigger                     bc4_remediation_trigger     NOT NULL,
    ...
    status                      bc4_remediation_status      NOT NULL DEFAULT 'open',
    assignee_id                 UUID,
    ...
    CONSTRAINT bc4_remediation_case_pk PRIMARY KEY (case_id)
);

COMMENT ON TABLE bc4_remediation_case IS
...
COMMENT ON COLUMN bc4_remediation_case.case_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc4_remediation_case.trigger IS
    'What caused this case to be opened. (G11, RC.3)';
COMMENT ON COLUMN bc4_remediation_case.linked_aggregate_ref IS
...
COMMENT ON COLUMN bc4_remediation_case.linked_corrective_event_id IS
...
COMMENT ON COLUMN bc4_remediation_case.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc4_remediation_case_status_idx  ON bc4_remediation_case (status);
CREATE INDEX bc4_remediation_case_trigger_idx ON bc4_remediation_case (trigger);
...
-- ---------------------------------------------------------------------------
-- bc5_master_agreement
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc5_master_agreement
...
CREATE TABLE bc5_master_agreement (
    agreement_id            UUID                                NOT NULL,
    party_id                UUID                                NOT NULL,
    party_type              bc5_master_agreement_party_type     NOT NULL,
    kind                    bc5_master_agreement_kind           NOT NULL,
    doc_hash                TEXT                                NOT NULL,
    status                  bc5_master_agreement_status         NOT NULL DEFAULT 'initiated',
    ...
    CONSTRAINT bc5_master_agreement_pk PRIMARY KEY (agreement_id)
);

COMMENT ON TABLE bc5_master_agreement IS
...
COMMENT ON COLUMN bc5_master_agreement.agreement_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc5_master_agreement.party_id IS
    'UUID of the signing party (supplier_id or investor_id depending on kind).';
COMMENT ON COLUMN bc5_master_agreement.doc_hash IS
    'Hash of the document sent to the signing vendor. Immutable once set. (C1)';
COMMENT ON COLUMN bc5_master_agreement.signature_cert_serial IS
    'Certificate serial number from e-sign vendor. Set exactly once on '
    'BC19 SignatureCompleted. (MA.3, DL-048, C2)';
COMMENT ON COLUMN bc5_master_agreement.stamp_cert_id IS
    'Stamping certificate ID from e-stamp vendor. Set exactly once on '
    'BC19 StampIssued. (MA.3, DL-048)';
COMMENT ON COLUMN bc5_master_agreement.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc5_master_agreement_party_idx ON bc5_master_agreement (party_id, kind);
CREATE INDEX bc5_master_agreement_status_idx ON bc5_master_agreement (status);


-- ---------------------------------------------------------------------------
-- bc5_assignment_set
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc5_assignment_set
...
CREATE TABLE bc5_assignment_set (
    ...
    CONSTRAINT bc5_assignment_set_pk PRIMARY KEY (assignment_set_id),
    CONSTRAINT bc5_assignment_set_listing_fk
        FOREIGN KEY (listing_id) REFERENCES bc1_listing (listing_id),
    -- AS.1: one AssignmentSet per listing. (C27, DL-002)
    CONSTRAINT bc5_assignment_set_listing_uidx UNIQUE (listing_id),
    -- AS.5: signed_count + unsigned_count = total_count
    CONSTRAINT bc5_assignment_counts_balance
        CHECK (signed_count + unsigned_count = total_count),
    CONSTRAINT bc5_assignment_counts_nonneg
        CHECK (signed_count >= 0 AND unsigned_count >= 0 AND total_count >= 0)
);

COMMENT ON TABLE bc5_assignment_set IS
...
COMMENT ON COLUMN bc5_assignment_set.assignment_set_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc5_assignment_set.sign_deadline IS
    'Timestamp after which signing attempts are rejected. '
    'Set by app layer as created_at + INTERVAL 24 hours. (G13, AS.2)';
COMMENT ON COLUMN bc5_assignment_set.legs IS
    'JSONB array of per-investor assignment legs. Each element: '
    '{investor_id, subscription_id, allocation_paise, signature_request_id?, '
    'doc_hash?, status: bc5_assignment_leg_status, signature_cert_serial?}. '
    'doc_hash immutable once set at AssignmentSignature.Initiated. (C1, C2, AS.6)';
COMMENT ON COLUMN bc5_assignment_set.signed_count IS
    'Count of legs with status = signed. Incremented on each RecordLegSigned.';
COMMENT ON COLUMN bc5_assignment_set.unsigned_count IS
    'Count of legs with status ∈ {pending, initiated, failed}. '
    'signed_count + unsigned_count = total_count at all times. (AS.5)';
COMMENT ON COLUMN bc5_assignment_set.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc5_assignment_set_status_idx ON bc5_assignment_set (status);


-- ---------------------------------------------------------------------------
-- bc5_signature_request
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc5_signature_request
...
CREATE TABLE bc5_signature_request (
    signature_request_id    UUID                            NOT NULL,
    signer_id               UUID                            NOT NULL,
    signer_type             bc5_signer_type                 NOT NULL,
    doc_hash                TEXT                            NOT NULL,
    -- Freeform reference to owning aggregate (master_agreement_id or
    -- assignment_set_id:investor_id composite). App layer encodes convention.
    parent_aggregate_ref    TEXT                            NOT NULL,
    status                  bc5_signature_request_status    NOT NULL DEFAULT 'initiated',
    ...
    CONSTRAINT bc5_signature_request_pk PRIMARY KEY (signature_request_id),
    -- SR.2: retry_count ≤ 3 (A2 §3.6)
    CONSTRAINT bc5_signature_request_retry_max
        CHECK (retry_count <= 3),
    CONSTRAINT bc5_signature_request_retry_nonneg
        CHECK (retry_count >= 0)
);

COMMENT ON TABLE bc5_signature_request IS
...
COMMENT ON COLUMN bc5_signature_request.signature_request_id IS
...
COMMENT ON COLUMN bc5_signature_request.parent_aggregate_ref IS
...
COMMENT ON COLUMN bc5_signature_request.retry_count IS
...
COMMENT ON COLUMN bc5_signature_request.cert_serial IS
    'Certificate serial from signing vendor. Set exactly once on terminal '
    'completed status. (SR.3, C2)';
COMMENT ON COLUMN bc5_signature_request.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

-- SR.1: uniqueness for idempotent replay (G18, A2 §3.3)
CREATE UNIQUE INDEX bc5_signature_request_idempotency_uidx
    ON bc5_signature_request (signer_id, doc_hash, parent_aggregate_ref);

CREATE INDEX bc5_signature_request_status_idx ON bc5_signature_request (status);
...
-- bc6_maturity_case
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc6_maturity_case
...
CREATE TABLE bc6_maturity_case (
    maturity_case_id            UUID                NOT NULL,
    listing_id                  UUID                NOT NULL,
    maturity_date               DATE                NOT NULL,
    delay_status                bc6_delay_status    NOT NULL DEFAULT 'on_track',
    ...
    CONSTRAINT bc6_maturity_case_pk PRIMARY KEY (maturity_case_id),
    CONSTRAINT bc6_maturity_case_listing_fk
        FOREIGN KEY (listing_id) REFERENCES bc1_listing (listing_id),
    -- MC.1: one MaturityCase per listing. (DL-028, DL-029)
    CONSTRAINT bc6_maturity_case_listing_uidx UNIQUE (listing_id),
    CONSTRAINT bc6_maturity_case_dpd_nonneg CHECK (days_past_due >= 0),
    CONSTRAINT bc6_maturity_case_recovery_nonneg CHECK (recovery_total >= 0)
);

COMMENT ON TABLE bc6_maturity_case IS
...
COMMENT ON COLUMN bc6_maturity_case.outcome IS
    ...
    '(MC.4, DL-029)';
COMMENT ON COLUMN bc6_maturity_case.panel_lawyer_assignment_id IS
    'Reference to assigned panel lawyer. Set when delay_status transitions '
    'to seriously_delayed (DPD 16+). (DL-028)';
COMMENT ON COLUMN bc6_maturity_case.recovery_total IS
...
COMMENT ON COLUMN bc6_maturity_case.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc6_maturity_case_delay_status_idx ON bc6_maturity_case (delay_status);
CREATE INDEX bc6_maturity_case_maturity_date_idx ON bc6_maturity_case (maturity_date);


-- ---------------------------------------------------------------------------
-- bc6_collections_action
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc6_collections_action
...
CREATE TABLE bc6_collections_action (
    action_id               UUID                            NOT NULL,
    maturity_case_id        UUID                            NOT NULL,
    action_type             bc6_collections_action_type     NOT NULL,
    ...
    CONSTRAINT bc6_collections_action_pk PRIMARY KEY (action_id),
    CONSTRAINT bc6_collections_action_case_fk
        FOREIGN KEY (maturity_case_id) REFERENCES bc6_maturity_case (maturity_case_id)
);

COMMENT ON TABLE bc6_collections_action IS
    'BC6 CollectionsAction. Immutable record of each manual collections activity '
    'against a MaturityCase. panel_lawyer_letter only when case is in '
    'seriously_delayed or under_adjudication. (DL-028, CA.1, CA.2)';

COMMENT ON COLUMN bc6_collections_action.action_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc6_collections_action.notes_doc_hash IS
    'Hash of the notes or evidence document attached to this action. '
    'Immutable once written. (C1, C2)';
COMMENT ON COLUMN bc6_collections_action.actor_id IS
    'Admin user (Ops Executive or Credit Reviewer) who recorded the action.';

CREATE INDEX bc6_collections_action_case_idx ON bc6_collections_action (maturity_case_id);
CREATE INDEX bc6_collections_action_type_idx ON bc6_collections_action (action_type);


-- ---------------------------------------------------------------------------
-- bc6_claim_case
-- ---------------------------------------------------------------------------
-- APPLICATION-LAYER INVARIANTS: bc6_claim_case
...
CREATE TABLE bc6_claim_case (
    claim_id                    UUID                NOT NULL,
    listing_id                  UUID                NOT NULL,
    maturity_case_id            UUID                NOT NULL,
    claim_type                  bc6_claim_type      NOT NULL,
    -- Immutable evidence document hash. (C1)
    evidence_doc_hash           TEXT                NOT NULL,
    status                      bc6_claim_status    NOT NULL DEFAULT 'raised',
    ...
    CONSTRAINT bc6_claim_case_pk PRIMARY KEY (claim_id),
    CONSTRAINT bc6_claim_case_listing_fk
        FOREIGN KEY (listing_id) REFERENCES bc1_listing (listing_id),
    CONSTRAINT bc6_claim_case_maturity_fk
        FOREIGN KEY (maturity_case_id) REFERENCES bc6_maturity_case (maturity_case_id),
    CONSTRAINT bc6_claim_indemnity_nonneg
        CHECK (supplier_indemnity_amount IS NULL OR supplier_indemnity_amount >= 0)
);

COMMENT ON TABLE bc6_claim_case IS
...
COMMENT ON COLUMN bc6_claim_case.claim_id IS
    'UUIDv7 primary key. Generated in app layer.';
COMMENT ON COLUMN bc6_claim_case.evidence_doc_hash IS
    'Hash of the evidence document submitted with the claim. '
    'Immutable once written. (C1)';
COMMENT ON COLUMN bc6_claim_case.outcome IS
    'Terminal outcome (bc6_maturity_outcome enum). NO DB DEFAULT. Set exclusively '
    'via BC3 DefaultCase.Classified subscriber. App layer must reject direct writes. '
    '(CCL.2, DL-029)';
COMMENT ON COLUMN bc6_claim_case.supplier_indemnity_amount IS
    'Supplier indemnity amount in paise for dilution or fraud claims. '
    'NULL for pure dispute claims. (CCL.3, Spec §2.2)';
COMMENT ON COLUMN bc6_claim_case.aggregate_version IS
    'Optimistic concurrency version. App layer increments on every write.';

CREATE INDEX bc6_claim_case_listing_idx      ON bc6_claim_case (listing_id);
CREATE INDEX bc6_claim_case_maturity_idx     ON bc6_claim_case (maturity_case_id);
CREATE INDEX bc6_claim_case_status_idx       ON bc6_claim_case (status);
CREATE INDEX bc6_claim_case_claim_type_idx   ON bc6_claim_case (claim_type);
