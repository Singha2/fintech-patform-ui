...
--
-- Scope: five tables only — tblIdentity, tblCredential, tblMfaFactor,
--        tblOtpChallenge, tblSession.
--
...
--   - One tblIdentity row per human/principal; multiple credentials per identity
--     are allowed for investors (password + Google on the same identity).
--   - Buyer acknowledgment users have NO tblCredential row — login is OTP only.
--   - Admins MUST have email+password tblCredential AND TOTP tblMfaFactor.
--   - Auditors get a provisioned password credential bounded by tblIdentity.
...
-- ============================================================================
-- tblIdentity — one row per authenticable principal
-- ============================================================================
CREATE TABLE tblIdentity (
    ...
    CONSTRAINT tblIdentity_email_unique UNIQUE (email),

    CONSTRAINT tblIdentity_validity_window_ordered
        CHECK (valid_from IS NULL OR valid_until IS NULL OR valid_from <= valid_until),

    CONSTRAINT tblIdentity_auditor_must_be_time_bound
        CHECK (kind <> 'auditor' OR (valid_from IS NOT NULL AND valid_until IS NOT NULL)),

    CONSTRAINT tblIdentity_phone_required_for_ack_user
        CHECK (kind <> 'acknowledgment_user' OR phone_e164 IS NOT NULL)
);

COMMENT ON TABLE tblIdentity IS
    'Authentication-layer identity. One row per authenticable principal regardless of kind. Domain tables (AdminUser, InvestorAccount, AcknowledgmentUser) reference identity_id; SupplierAccount does not (no supplier login in Phase 1).';

COMMENT ON COLUMN tblIdentity.identity_id IS
    'Primary identity key. UUIDv7 generated at the application layer (B3 root-identity convention).';
COMMENT ON COLUMN tblIdentity.kind IS
...
COMMENT ON COLUMN tblIdentity.email IS
...
COMMENT ON COLUMN tblIdentity.phone_e164 IS
...
COMMENT ON COLUMN tblIdentity.status IS
    'Lifecycle state of the identity itself. Distinct from the domain account''s onboarding state machine — an investor may be identity.status=active but InvestorAccount.status=kyc_in_review.';

COMMENT ON CONSTRAINT tblIdentity_auditor_must_be_time_bound ON tblIdentity IS
...
COMMENT ON CONSTRAINT tblIdentity_phone_required_for_ack_user ON tblIdentity IS
...
-- ============================================================================
-- tblCredential — long-lived authentication material
-- ============================================================================
CREATE TABLE tblCredential (
    credential_id     UUID         PRIMARY KEY,
    identity_id       UUID         NOT NULL REFERENCES tblIdentity(identity_id) ON DELETE RESTRICT,
    ...
    CONSTRAINT tblCredential_password_shape
        CHECK (kind <> 'password' OR (secret_hash IS NOT NULL AND subject_id IS NULL)),

    CONSTRAINT tblCredential_oauth_google_shape
        CHECK (kind <> 'oauth_google' OR (subject_id IS NOT NULL AND secret_hash IS NULL)),

    CONSTRAINT tblCredential_oauth_subject_unique
        UNIQUE (kind, subject_id)
);

COMMENT ON TABLE tblCredential IS
    'Long-lived authentication material. One identity_id may carry multiple credentials (e.g. an investor with both a password and Google OAuth — same identity_id, two rows). Buyer acknowledgment users have ZERO rows here (DL-021: email+OTP only). Admins have exactly one password row; their MFA factor lives in tblMfaFactor.';

COMMENT ON COLUMN tblCredential.credential_id IS 'Surrogate key. UUIDv7.';
COMMENT ON COLUMN tblCredential.identity_id IS 'Identity owning this credential. ON DELETE RESTRICT — identities are archived, never hard-deleted, to preserve audit-log references (C1, DL-040).';
COMMENT ON COLUMN tblCredential.kind IS 'Credential mechanism. Drives which of secret_hash / subject_id is populated (enforced by CHECK).';
COMMENT ON COLUMN tblCredential.secret_hash IS 'Argon2id / bcrypt hash for password credentials. Application is responsible for the algorithm, parameters, and pepper. NULL for oauth_google.';
COMMENT ON COLUMN tblCredential.subject_id IS 'Identity-provider subject (Google ''sub'' claim) for oauth_google. Unique per (kind, subject_id) so the same Google account cannot be linked to two platform identities. NULL for password.';
COMMENT ON COLUMN tblCredential.last_used_at IS 'Stamped on every successful authentication via this credential. Used by anomaly detection and credential-rotation hygiene reports.';
COMMENT ON COLUMN tblCredential.revoked_at IS 'Soft-revocation timestamp. A revoked row is retained for audit; the partial-unique constraints above only apply to non-revoked rows so a user may rotate credentials.';

COMMENT ON CONSTRAINT tblCredential_password_shape ON tblCredential IS
    'Password credentials carry secret_hash and never subject_id.';
COMMENT ON CONSTRAINT tblCredential_oauth_google_shape ON tblCredential IS
    'OAuth Google credentials carry the provider subject_id and never a secret_hash.';
COMMENT ON INDEX tblCredential_one_password_per_identity IS
...
COMMENT ON INDEX tblCredential_one_google_per_identity IS
...
COMMENT ON CONSTRAINT tblCredential_oauth_subject_unique ON tblCredential IS
...
-- ============================================================================
-- tblMfaFactor — second factors registered against an identity
-- ============================================================================
CREATE TABLE tblMfaFactor (
    ...
    CONSTRAINT tblMfaFactor_totp_has_secret
        CHECK (kind <> 'totp' OR secret_encrypted IS NOT NULL)
);

COMMENT ON TABLE tblMfaFactor IS
...
COMMENT ON COLUMN tblMfaFactor.kind IS 'TOTP preferred for admins (DL-035); SMS-OTP is fallback. SMS-OTP factor row exists to record enrolment intent; the actual one-time codes flow through tblOtpChallenge.';
COMMENT ON COLUMN tblMfaFactor.secret_encrypted IS 'TOTP shared secret, encrypted at the application layer with a KMS-managed key (Spec §7.3). NULL for sms_otp (the phone number lives on tblIdentity).';
COMMENT ON COLUMN tblMfaFactor.label IS 'User-friendly factor label (e.g. "Authenticator on Pixel 7"). Surfaced in the admin self-service factor list.';
COMMENT ON COLUMN tblMfaFactor.revoked_at IS 'Soft-removal timestamp. Revoking the last active factor for an admin must trigger application-layer status downgrade — see APPLICATION-LAYER INVARIANTS.';

COMMENT ON INDEX tblMfaFactor_one_active_totp_per_identity IS
...
-- ============================================================================
-- tblOtpChallenge — short-lived one-time codes for four purposes
-- ============================================================================
CREATE TABLE tblOtpChallenge (
    ...
    CONSTRAINT tblOtpChallenge_expiry_after_issue
        CHECK (expires_at > issued_at),

    CONSTRAINT tblOtpChallenge_attempts_nonneg
        CHECK (attempts >= 0 AND attempts <= max_attempts),

    CONSTRAINT tblOtpChallenge_consumed_implies_status
        CHECK ((consumed_at IS NULL) = (status <> 'consumed')),

    CONSTRAINT tblOtpChallenge_assertion_only_when_consumed
        CHECK (assertion_id IS NULL OR status = 'consumed')
);

COMMENT ON TABLE tblOtpChallenge IS
    'One-time codes spanning four purposes: login_mfa, phone_verify, email_verify, password_reset. Exactly one active challenge per (identity_id, purpose) — issuing a new one supersedes the prior (application transitions the prior to status=superseded before insert). The assertion_id minted on consumption is what session.mfa_assertion_id references for login_mfa.';

COMMENT ON COLUMN tblOtpChallenge.purpose IS 'Why this OTP exists. The four purposes are the complete Phase 1 set; new purposes require enum-extension and a migration.';
COMMENT ON COLUMN tblOtpChallenge.code_hash IS 'Hash of the OTP code (never the cleartext). The plaintext is delivered to the user once and never persisted.';
COMMENT ON COLUMN tblOtpChallenge.delivery_channel IS 'Which channel the code was dispatched to: email | sms. Free-text rather than enum since BC15 owns channel semantics and may extend.';
COMMENT ON COLUMN tblOtpChallenge.attempts IS 'Wrong-code verification attempts to date. On reaching max_attempts the application transitions status to expired and forces re-issuance.';
COMMENT ON COLUMN tblOtpChallenge.expires_at IS 'Hard TTL. Typical values: login_mfa 5m; phone_verify 10m; email_verify 30m; password_reset 30m — final values owned by BC10 policy.';
COMMENT ON COLUMN tblOtpChallenge.assertion_id IS 'For purpose=login_mfa: the assertion identifier minted when this challenge is consumed. Referenced by tblSession.mfa_assertion_id and by every admin envelope''s actor.mfa_assertion_id (B2 §2.2, AU10.3). NULL for non-login_mfa purposes.';

COMMENT ON CONSTRAINT tblOtpChallenge_consumed_implies_status ON tblOtpChallenge IS
    'consumed_at and status=''consumed'' are tied together — neither can be set without the other.';
COMMENT ON INDEX tblOtpChallenge_one_active_per_identity_purpose IS
...
-- ============================================================================
-- tblSession — authenticated session state
-- ============================================================================
CREATE TABLE tblSession (
    ...
    CONSTRAINT tblSession_absolute_after_issue
        CHECK (absolute_expires_at > issued_at),

    CONSTRAINT tblSession_idle_within_absolute
        CHECK (idle_expires_at <= absolute_expires_at),

    CONSTRAINT tblSession_revoked_implies_status
        CHECK ((revoked_at IS NULL) OR (status = 'revoked'))
);

COMMENT ON TABLE tblSession IS
...
COMMENT ON COLUMN tblSession.session_id IS 'Opaque session identifier. The sole value carried in the session cookie; cookie is HttpOnly, Secure, SameSite=Lax (Spec §7.3).';
COMMENT ON COLUMN tblSession.idle_expires_at IS 'Rolling idle timeout. Refreshed on each successful request; if now() > idle_expires_at the application transitions status to expired.';
COMMENT ON COLUMN tblSession.absolute_expires_at IS 'Hard ceiling. The session cannot be extended past this point regardless of activity; re-authentication is required.';
COMMENT ON COLUMN tblSession.mfa_assertion_id IS 'For admin sessions: references the consumed tblOtpChallenge.assertion_id from the most-recent successful TOTP/SMS verification. Admin command handlers reject when this is NULL or when the underlying challenge''s consumed_at is older than the policy freshness window (B4 §6.4 — 5 minutes for sensitive actions, 30 minutes for normal).';
COMMENT ON COLUMN tblSession.tenant_claims IS 'IAM-issued claims serialised at session establishment (B2 §3.10 TenantClaim.Issued, G19). For investor: {investor_id}; supplier_user: {supplier_id} (dormant in Phase 1); ack_user: {buyer_id}; admin: {roles:[…]}; auditor: {scope_id}. Repository injects these as query predicates — never trusted from client input.';
COMMENT ON COLUMN tblSession.client_ip IS 'IP at session issuance, captured for audit (C1) and anomaly detection. Not used for binding — sessions survive IP changes.';

COMMENT ON CONSTRAINT tblSession_idle_within_absolute ON tblSession IS
...
--    This section ONLY adds the FK to tblIdentity — do NOT re-add the
--    column or UNIQUE (those ALTERs removed per schema-fix session).
--    (AdminUser, InvestorAccount, AcknowledgmentUser only — DL-012 keeps
--     SupplierAccount login-less in Phase 1, so no FK there.)
--    FLYWAY ORDERING: this file must run AFTER 01_core.sql and
--    02_counterparty_platform.sql (FK targets must exist first).
-- -----------------------------------------------------------------------------

-- identity_id column + tblAdminUser_identity_uq UNIQUE already defined in
-- 02_counterparty_platform.sql CREATE TABLE tblAdminUser. FK only added here.
ALTER TABLE tblAdminUser
    ADD CONSTRAINT tblAdminUser_identity_fk
        FOREIGN KEY (identity_id) REFERENCES tblIdentity(identity_id) ON DELETE RESTRICT;

COMMENT ON COLUMN tblAdminUser.identity_id IS
    'Auth-layer join key. The admin''s tblIdentity row carries kind=admin_user. Domain row carries roles, SoD acknowledgements, deviation register links (B3 §2.10).';


-- identity_id column + tblInvestorAccount_identity_uq UNIQUE already defined in
-- 02_counterparty_platform.sql CREATE TABLE tblInvestorAccount. FK only added here.
ALTER TABLE tblInvestorAccount
    ADD CONSTRAINT tblInvestorAccount_identity_fk
        FOREIGN KEY (identity_id) REFERENCES tblIdentity(identity_id) ON DELETE RESTRICT;

COMMENT ON COLUMN tblInvestorAccount.identity_id IS
    'Auth-layer join key. The investor''s tblIdentity row carries kind=investor. Domain row carries the onboarding state machine (Spec §6.3) and the suitability/financial-profile chain).';


-- identity_id column + tblAcknowledgmentUser_identity_uq UNIQUE already defined in
-- 02_counterparty_platform.sql CREATE TABLE tblAcknowledgmentUser. FK only added here.
ALTER TABLE tblAcknowledgmentUser
    ADD CONSTRAINT tblAcknowledgmentUser_identity_fk
        FOREIGN KEY (identity_id) REFERENCES tblIdentity(identity_id) ON DELETE RESTRICT;

COMMENT ON COLUMN tblAcknowledgmentUser.identity_id IS
    'Auth-layer join key. The ack user''s tblIdentity row carries kind=acknowledgment_user; no tblCredential row exists for it (DL-021: email+OTP login only). The buyer relationship lives on the domain row (BC9 AU.1: belongs to exactly one buyer).';
...
-- The UNIQUE constraint on tblIdentity.email already provides this index; no
...
CREATE INDEX idx_tblIdentity_kind_status
    ON tblIdentity (kind, status);

-- Auditor auto-disable scheduler scan: identities expiring soon
CREATE INDEX idx_tblIdentity_valid_until
    ON tblIdentity (valid_until)
...
CREATE UNIQUE INDEX tblCredential_one_password_per_identity
    ON tblCredential (identity_id)
    WHERE kind = 'password' AND revoked_at IS NULL;

-- At most one active Google OAuth linkage per identity
CREATE UNIQUE INDEX tblCredential_one_google_per_identity
    ON tblCredential (identity_id)
    WHERE kind = 'oauth_google' AND revoked_at IS NULL;

-- At most one active TOTP factor per identity
CREATE UNIQUE INDEX tblMfaFactor_one_active_totp_per_identity
    ON tblMfaFactor (identity_id)
    WHERE kind = 'totp' AND revoked_at IS NULL;

-- At most one active OTP challenge per (identity, purpose)
CREATE UNIQUE INDEX tblOtpChallenge_one_active_per_identity_purpose
    ON tblOtpChallenge (identity_id, purpose)
    WHERE status = 'active';

-- Credential lookups
CREATE INDEX idx_tblCredential_identity
    ON tblCredential (identity_id)
...
CREATE INDEX idx_tblCredential_oauth_subject_active
    ON tblCredential (subject_id)
    WHERE kind = 'oauth_google' AND revoked_at IS NULL;

-- MFA factor lookup at challenge issuance
CREATE INDEX idx_tblMfaFactor_identity_active
    ON tblMfaFactor (identity_id, kind)
    WHERE revoked_at IS NULL;

-- OTP challenge lookup: verify code by (identity, purpose, active)
CREATE INDEX idx_tblOtpChallenge_lookup
    ON tblOtpChallenge (identity_id, purpose)
    WHERE status = 'active';

-- OTP expiry sweep (background job moves expired actives to status=expired)
CREATE INDEX idx_tblOtpChallenge_expiry_sweep
    ON tblOtpChallenge (expires_at)
    WHERE status = 'active';

-- Session lookup by id is via PK. Index for the "all active sessions for an
-- identity" admin screen (revoke-all, anomaly review).
CREATE INDEX idx_tblSession_identity_active
    ON tblSession (identity_id)
    WHERE status = 'active';

-- Session idle/absolute expiry sweep
CREATE INDEX idx_tblSession_expiry_sweep
    ON tblSession (idle_expires_at)
    WHERE status = 'active';
...
--     The combinations of tblIdentity.kind, tblCredential rows, and
--     tblMfaFactor rows MUST satisfy:
--
--       admin_user           : exactly 1 active password tblCredential
--                              AND at least 1 active TOTP tblMfaFactor
--                              (DL-035, AU10.2).
--       investor             : at least 1 active tblCredential row, kind in
--                              {password, oauth_google}; both allowed on
--                              the same identity_id.
--       acknowledgment_user  : ZERO tblCredential rows; ZERO tblMfaFactor rows.
--                              Login is OTP-only via tblOtpChallenge
--                              purpose=login_mfa (DL-021).
--       auditor              : exactly 1 active password tblCredential
...
--     when at least one active tblMfaFactor row exists for that identity_id
...
--     session.mfa_assertion_id IS NOT NULL AND points to a tblOtpChallenge
...
--     Before INSERTing a new tblOtpChallenge for an existing
...
--     tblCredential row, log in as that identity. If the sub is unknown but
--     the email matches an existing tblIdentity, the application — NOT the
...
--     A scheduler MUST scan tblIdentity for kind='auditor' AND
--     status='active' AND valid_until <= now() and transition to
--     status='auto_disabled', revoking all active tblSession rows for that
...
--     tblSession.tenant_claims MUST be populated by the IAM layer at session
...
--     Revoking any tblCredential row or tblMfaFactor row for an identity
--     SHOULD trigger revocation of that identity''s active tblSession rows.
...
--     There is intentionally no identity_id column on tblSupplierAccount
